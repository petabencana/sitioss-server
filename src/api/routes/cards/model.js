...
export default (config, db, logger) => ({
  ...
  submitReport: (card, body) =>
    new Promise((resolve, reject) => {
      ...
      let queries = [
        {
          ...
          (card_id, card_data, text, created_at, disaster_type,
            partner_code, status,
            the_geom)
          VALUES ($1, $2, COALESCE($3,''), $4, COALESCE($5,null), COALESCE($6,null), $7,
      ...
      db.tx((t) => {
        return t.batch(
          queries.map((query) => t.oneOrNone(query.query, query.values))
        );
      })
        .timeout(config.PGTIMEOUT)
        ...
        /* istanbul ignore next */
        .catch((err) => {
          console.log("🚀 ~ file: model.js ~ line 148 ~ newPromise ~ err", err);
          /* istanbul ignore next */
          reject(err);
        });
    }),
  ...
  updateReport: (card, body) =>
    new Promise((resolve, reject) => {
      ...
      db.tx((t) => {
        return t.batch(
          queries.map((query) => t.none(query.query, query.values))
        );
      })
        .timeout(config.PGTIMEOUT)
        .then((data) => resolve(data))
