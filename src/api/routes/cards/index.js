...
export default ({ config, db, logger }) => {
  ...
  api.head(
    "/:cardId",
    cacheResponse(config.CACHE_DURATION_CARDS),
    validate({
  ...
  api.get("/expiredcards", (req, res, next) => {
    cards(config, db, logger)
      .expiredCards()
      .then((data) => {
      ...
      body: Joi.object().keys({
        ...
        card_data: Joi.object()
          .keys({
            ...
            accessabilityFailure: Joi.number()
              .integer()
              .min(0)
              .max(7)
        ...
        created_at: Joi.alternatives(
          Joi.date().format("YYYY-MM-DDTHH:mm:ssZ"),
          Joi.date().format("YYYY-MM-DDTHH:mm:ss.SSSZ")
        ).required(),
        location: Joi.object()
          .required()
          .keys({
        ...
        cards(config, db, logger)
          ...
          .then((card) => {
            ...
            if (!card) {
              ...
              createReport(config, db, logger, card, req, notify, res, next);
          });
      } catch (err) {
        /* istanbul ignore next */
        logger.error(err);
